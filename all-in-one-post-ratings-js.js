...
function aio_rating_hover(post_id,rat,p_type){
...
function aio_rating_none(post_id,rat,p_type){
	var curent_img = $('#'+post_id+'_'+rat).attr('src');
...
function give_rate(el,post_id,rat,p_type){
	aio_rating_hover(post_id,rat,p_type)
	console.log(el);
	el.parent('div#all-in-one-rating').addClass('active');
	el.parent('div#all-in-one-rating').find('div#aio-rating-loader').show();
	//alert(post_id+','+rat);
	jQuery.ajax({		
	  type:"POST",
	  ...
	  success:function(data){	  	
	  	$('div#all-in-one-rating div#aio-rating-loader').hide();
		  	$('div#all-in-one-rating.active').html(data);
			$('div#all-in-one-rating').removeClass('active');
	  }
	}); 
}
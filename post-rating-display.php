...
echo "<div id='all-in-one-rating' ".$itemtype_of_content."data-page-type='".$page_type."'>";	
	...
	if(!wp_is_post_revision($post)){
		...
		if($page_type =="category") {
			$allready_rate = get_term_meta($ratings_post_id, 'aio_rating_count',true);
		...
		if(!empty($allready_rate) && $allready_rate > 0){
			...
			aoi_post_rating_result($ratings_post_id, $total_rating, $rating_none_img, $rating_full_img, $rating_avg_img, $rating_hover_img, $page_type);
			//echo $page_type;
		} else {
			aoi_post_rating_none($ratings_post_id, $total_rating, $rating_none_img, $rating_full_img, $rating_avg_img, $rating_hover_img, $page_type);
			//echo $page_type;
		}
...
if('on' == $google_rich) {
	...
	echo $post_meta;
		...
		$aio_ratings_meta .= '<meta itemprop="ratingValue" content="'.get_term_meta($ratings_post_id,'aio_average_rating',true).'" />';
		$aio_ratings_meta .= '<meta itemprop="ratingCount" content="'.get_term_meta($ratings_post_id,'aio_rating_count',true).'" />';
